...
import { Plus, Star, Clock, CheckSquare, ArrowRight, Zap } from 'lucide-react';
import { mockDeliveryPartners, performanceData } from '../data/mockData';
...
export default function DeliveryPartners() {
  ...
  const filtered = mockDeliveryPartners.filter(p => {
    if (activeTab === 'Top Rated') return p.rating >= 4.9;
    if (activeTab === 'In-Transit') return p.status === 'In Delivery';
    return true;
  });

  const getStatusColor = (status) => {
    const colors = { 'In Delivery': 'bg-accent-green/15 text-accent-green', 'Available': 'bg-accent-blue/15 text-accent-blue', 'Inactive': 'bg-dark-300/30 text-text-muted', 'Returning': 'bg-accent-amber/15 text-accent-amber' };
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="col-span-2 glass-card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-text-secondary">94.8% AVG</span>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="glass-card p-6 text-center">
            ...
            <p className="text-xs text-accent-green mt-2">↗ +1.2% from yesterday</p>
          </div>
          <div className="glass-card p-6 bg-gradient-to-br from-purple-900/40 to-dark-600">
            <div className="flex items-center gap-2 mb-2">
              <CheckSquare size={18} className="text-purple-400" />
              <p className="text-xs text-text-muted uppercase tracking-widest font-semibold">Assign Orders</p>
            </div>
            <p className="text-2xl font-bold text-white mb-3">42 Pending Tasks</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-text-secondary text-sm">84 Partners Online</span>
          </div>
          ...
          {filtered.map((partner, i) => (
            <div key={partner.id} className="glass-card p-5 flex items-center gap-4 hover:border-purple-500/30 transition-all slide-up" style={{ animationDelay: `${i * 80}ms` }}>
              <div className="relative">
                <img src={partner.image} alt={partner.name} className="w-16 h-16 rounded-2xl object-cover" />
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <h4 className="text-base font-semibold text-white">{partner.name}</h4>
                  <div className="flex items-center gap-1 text-accent-amber">
                    <Star size={12} fill="currentColor" />
                    <span className="text-xs font-semibold">{partner.rating}</span>
                  </div>
                </div>
                <p className="text-xs text-text-muted mb-2">PARTNER ID: #{partner.id}</p>
                <div className="flex items-center gap-6">
                  <div>
                    <p className="text-[10px] text-text-muted uppercase tracking-wider">Total Orders</p>
                    <p className="text-lg font-bold text-white">{partner.totalOrders.toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-text-muted uppercase tracking-wider">Status</p>
                    <span className={`status-badge ${getStatusColor(partner.status)}`}>{partner.status}</span>
                  </div>
                </div>
              </div>
              <button className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center text-purple-400 hover:bg-purple-500/20 transition-colors">
                <Zap size={18} />
              </button>
            </div>
          ))}
