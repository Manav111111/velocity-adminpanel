  ...
  return (
    <div className="min-h-screen bg-dark-800 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="glass-card-solid p-8 w-full max-w-md relative slide-up">
        ...
        <h2 className="text-xl font-semibold text-white mb-1 text-center">Create Account</h2>
        <p className="text-text-secondary text-sm mb-6 text-center">Set up your admin access</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@velocitypro.com" className="glass-input w-full pl-11" required />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 6 characters" className="glass-input w-full pl-11 pr-11" required />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-primary">
                {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            </div>
          </div>
          <div>
            <label className="text-xs text-text-secondary uppercase tracking-wider font-semibold mb-1.5 block">Confirm Password</label>
            <div className="relative">
              <Lock size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted" />
              <input type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repeat password" className="glass-input w-full pl-11" required />
            </div>
          </div>
